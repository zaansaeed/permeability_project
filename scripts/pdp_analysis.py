...
X      = pd.read_csv(folder + "/models_and_training_data/X.csv")
rf_obj = joblib.load(folder + "/models_and_training_data/random_forest_model.joblib")
...
def plot_3way_pdp(feat1, feat2, feat3, n_grid=25, n_slices=6, n_sample=500):
    """
    True 3-way PDP: for each (feat1, feat2, feat3) grid point, average
    predictions over n_sample rows of the real data distribution.

    Parameters
    ----------
    feat1, feat2 : features shown on x/y axes of each subplot
    feat3        : feature sliced across subplots
    n_grid       : resolution of the feat1 × feat2 grid
    n_slices     : number of feat3 quantile slices
    n_sample     : rows of X used for marginalization (subsample for speed)
    """
    ...
    # subsample for speed while keeping true marginalization
    ...
    for idx, (f3_val, preds) in enumerate(zip(f3_vals, all_preds)):
        ...
        axes[idx].scatter(
        X[feat1], X[feat2],
        c='white', s=5, alpha=0.3, zorder=5)
        ...
        plt.colorbar(ct, ax=axes[idx], label="Predicted Permeability")
...
plot_3way_pdp('Pos_2_logP', 'Pos_1_logP', 'Pos_6_logP', n_slices = 8)
